...
- `bless_data.gd` (`BlessCardData`): 9 Bless card definitions (Side I and Side II), deck creation, flip logic, serialization.
...
- Each killed enemy grants its `xp_reward` (a float) to every living player.
- XP thresholds are `[0, 2, 5, 9, 14]`, so levels 2-5 require 2, 5, 9, and 14 total XP.
- Nashrat and Ashen Skeleton award 0.5 XP each; all other enemies award whole numbers.
...
| Hymn of Reckoning | 3 | 7 | Attack 5, Range 3. Increase damage by +2 for every Bless card currently held by allies in the party. | Magic ranged attack 5; party Bless bonus implemented via party_bless_count() |
...
| Apotheosis Strike | 4 | 7 | Attack 6, Range 3. Increase damage by +2 for every Bless card currently held by allies in the party. | Magic ranged attack 6; party Bless bonus implemented via party_bless_count() |
...
- `attack`: supports physical/magic damage, melee/ranged targeting, adjacent AoE, all-enemies-in-range AoE, ignore block, undead bonus, undead multiplier, party Bless count bonus, and applying conditions.
...
- `bless`: calls `_grant_bless(target)` which takes the next available market card and adds it to target's `bless_cards`.
...
- `party_bless_bonus`: implemented — sums `bless_cards.size()` for all living players via `bs.party_bless_count()`, multiplied by the `party_bless_bonus` value on the effect dict.
...
| Nashrat | 3 | 0 | 0 | 0.5 | Nashrat deck |
| AshenSkeleton | 5 | 2 | 0 | 0.5 | Ashen Skeleton deck |
...
- Enemy damage is reduced by block only for physical attacks; magic attacks ignore block.
...
| Shield Bash | 7 | Move 2, Attack 4, Stun | Move 2; melee attack 4; apply Stun |
...
| Shield Bash | 6 | Move 2, Stun if adj | Move 2; apply Stun if adjacent |
...
| Bite | 3 | Move 3, Attack 1 | Move 3; melee attack 1 |
| Swarm | 4 | Attack 2 | Melee attack 2 if adjacent |
| Frenzied Rush | 3 | Move 3, Attack 1 | Move 3; melee attack 1 |
| Diseased Bite | 4 | Attack 1, Poison | Melee attack 1; apply Poison |
| Retreat | 2 | Move 2 away | Move away from nearest player |
...
- `ignore_block` bypasses block on physical attacks.
- Block is not capped.
...
- `Bless`: Clerics hold up to 3 Bless cards (BlessCardData) in `bless_cards`. Cards are acquired from the shared 3-slot market via card effects. Played during own SELECT turn for immediate or pending effects. Side I flips to Side II (returned to draw pile bottom); Side II is permanently discarded. Pending effects (damage bonus, ignore-block, reflect) are consumed or cleared at round end.
...
- `BattleState.to_dict()` serializes player count, players, enemies, selected planning player, phase, round number, combat log, `bless_market` (3 slots), `bless_draw`, and `bless_discard_count`.
- `PlayerState.to_dict()` serializes identity, level/XP/resources, position, deck zones, readiness, alive state, `bless_cards` (name+side dicts), pending bless flags, `stamina_bonus`, conditions, burn, and damage immunity.
- `EnemyState.to_dict()` serializes type, HP, XP reward, position, alive state, behavior zones, revealed behavior name, and conditions.
